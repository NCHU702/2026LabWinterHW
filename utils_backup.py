...
def visualize_sequence_to_gif(
    ...
    # 設定標題和標籤
    axes[0].set_title('降雨量 (mm)', fontsize=14, fontweight='bold')
    axes[1].set_title('淹水深度 (mm)', fontsize=14, fontweight='bold')
    ...
    # 加入色條
    if show_colorbar:
        plt.colorbar(rain_im, ax=axes[0], fraction=0.046, pad=0.04, label='mm')
        plt.colorbar(flood_im, ax=axes[1], fraction=0.046, pad=0.04, label='mm')
    ...
    def update(frame_idx):
        ...
        if t < len(rain_files):
            ...
            axes[0].set_title(f'降雨量 (mm) - 時刻 {t}', fontsize=14, fontweight='bold')
        
        # 更新淹水深度
        if t < len(flood_files):
            flood_data = flood_data_list[frame_idx] if frame_idx < len(flood_data_list) else np.zeros_like(rain_data_list[0])
            flood_im.set_data(flood_data)
            axes[1].set_title(f'淹水深度 (mm) - 時刻 {t}', fontsize=14, fontweight='bold')
...
if __name__ == "__main__":
    ...
    if os.path.exists(test_folder):
        ...
        # 生成範例 GIF (前 10 個時刻)
        try:
            output_gif = os.path.join(vis_dir, 't12_demo.gif')
